...
class Bill_App:
    def __init__(self, root):
        self.root = root  # intiialize the root
        self.root.geometry("1350x700+0+0")
        self.root.title("Billing software")
        # frame
        bg_color = "#074463"
        title = Label(self.root, text="Billing software", bd=12, relief=GROOVE, bg=bg_color,font=("times new roman", 30, "bold"), pady=2).pack(fill=X)
        # ================variables=============
        #=================Cosmetics============
        self.soap=IntVar()
        ...
        self.fanta=IntVar()
        #================total product price & tax Vaiable====
        ...
        F1.place(x=0, y=80, relwidth=1)  # width = width of self root
        ...
        F2 = LabelFrame(self.root, bd=10, relief=GROOVE, text="Cosmetics", font=("times new roman", 15, "bold"),
                        fg="gold", bg=bg_color)
        F2.place(x=5, y=180, width=325, height=380)  # width = width of self root
        ...
        hair_s_lbl = Label(F2, text="hair spary", font=("times new roman", 16, "bold"), bg=bg_color,fg="lightgreen").grid(row=3, column=0, padx=10, pady=10, sticky="w")
        ...
        # ===========Grocery Frame ======g1========
        F3 = LabelFrame(self.root, bd=10, relief=GROOVE, text="Grocery product", font=("times new roman", 15, "bold"),fg="gold", bg=bg_color)
        F3.place(x=340, y=180, width=325, height=380)  # width = width of self root
        ...
        F4 = LabelFrame(self.root, bd=10, relief=GROOVE, text="Cold drink", font=("times new roman", 15, "bold"),
                        fg="gold", bg=bg_color)
        F4.place(x=670, y=180, width=325, height=380)  # width = width of self root
        ...
        c2_txt = Entry(F4, width=10,textvariable=self.maza, font=("times new roman", 16, "bold"), bd=5, relief=SUNKEN).grid(row=1, column=1,padx=10, pady=10)

        c3_lbl = Label(F4, text="thums up", font=("times new roman", 16, "bold"), bg=bg_color, fg="lightgreen").grid(row=2, column=0, padx=10, pady=10, sticky="w")
        c3_txt = Entry(F4, width=10,textvariable=self.thums_up, font=("times new roman", 16, "bold"), bd=5, relief=SUNKEN).grid(row=2, column=1,padx=10, pady=10)

        c4_lbl = Label(F4, text="7 up", font=("times new roman", 16, "bold"), bg=bg_color, fg="lightgreen").grid(row=3,column=0,padx=10,pady=10,sticky="w")
        c4_txt = Entry(F4, width=10,textvariable=self.seven_up, font=("times new roman", 16, "bold"), bd=5, relief=SUNKEN).grid(row=3, column=1,padx=10, pady=10)

        c5_lbl = Label(F4, text="fanta", font=("times new roman", 16, "bold"), bg=bg_color, fg="lightgreen").grid(row=4,column=0,padx=10,pady=10,sticky="w")
        c5_txt = Entry(F4, width=10,textvariable=self.fanta, font=("times new roman", 16, "bold"), bd=5, relief=SUNKEN).grid(row=4, column=1,padx=10, pady=10)

        c6_lbl = Label(F4, text="leeche", font=("times new roman", 16, "bold"), bg=bg_color, fg="lightgreen").grid(row=5, column=0, padx=10, pady=10, sticky="w")
        c6_txt = Entry(F4, width=10,textvariable=self.leeche, font=("times new roman", 16, "bold"), bd=5, relief=SUNKEN).grid(row=5, column=1,padx=10, pady=10)
        ...
        Exit_btn=Button(btn_f,text="Exit",command=self.Exit_app,bg="cadetblue",fg="white",bd=2,pady=15,width=9,font="arial 15 bold").grid(row=0,column=3,padx=5,pady=5)
        self.welcome_bill()
    ...
    def total(self):
        ...
        self.d_se_p=(self.seven_up.get() * 30)
        self.d_f_p=(self.fanta.get() * 40)
        self.d_l_p=(self.leeche.get() * 60)
        self.d_t_p=(self.thums_up.get() * 60)
        ...
        self.cold_drink_price.set("Rs. "+str(self.total_cosmetic_price))
    ...
    def welcome_bill(self):
        ...
        self.txtarea.insert(END,"\t Welcome  my  friend\n")
    ...
    def bill_area(self):
        if self.c_name.get()=="" or self.c_phon.get()=="":
        ...
        elif self.cosmetic_price.get()=="Rs. 0.0" and self.grocery_price.get()=="Rs. 0.0" and self.cold_drink_price=="Rs. 0.0":
            messagebox.showerror("Error","No Product purchases")
        else:
            self.welcome_bill()
           #=============cosmetics============
            if self.soap.get()!=0:
           ...
           #=============cold drink============
            ...
            if self.thums_up.get()!=0:
                self.txtarea.insert(END,f"\n thums_up \t\t{self.thums_up.get()}\t\t{self.d_t_p}")
            if self.seven_up.get()!=0:
                self.txtarea.insert(END,f"\n seven_up \t\t{self.seven_up.get()}\t\t{self.d_se_p}")
            if self.fanta.get()!=0:
                self.txtarea.insert(END,f"\n fanta \t\t{self.fanta.get()}\t\t{self.d_f_p}")
            if self.leeche.get()!=0:
                self.txtarea.insert(END,f"\n leeche \t\t{self.leeche.get()}\t\t{self.d_l_p}")
    ...
    def clear_data(self):
        op=messagebox.askyesno("Exit","Do u clr the data?")
        if op>0:
            self.soap.set(0)
